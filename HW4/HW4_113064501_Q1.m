...
for l_idx = 1:length(L_vals)
    ...
    for snr_idx = 1:length(Es_N0_dB)
        ...
        % Multiple branch Rayleigh fading
        h = (1/sqrt(2)) * (randn(L, numBits) + 1i*randn(L, numBits)); % Rayleigh channel for each branch
...
legend show;
title('Selective Combining (SC, Rayleigh)');
...
ylim([1e-6, 1]); % Set y-axis limits
legend show;
title('Maximal Ratio Combining (MRC, Rayleigh)');
...
ylim([1e-6, 1]); % Set y-axis limits
legend show;
title('Equal Gain Combining (EGC, Rayleigh)');
...
ylim([1e-6, 1]); % Set y-axis limits
legend show;
title('Direct Combining (DC, Rayleigh)');
